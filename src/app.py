...
load = compressed_files(filename, path=extract, sep="|").rename(
...
change = st.text_input("Change Group Name: ", placeholder=select_skill.skill)
...
def make_change(change_type, choice, c):
    with c:
        change = st.multiselect(
            change_type, choice, default=select_skill.filters[change_type]
        )
        if change:
    ...
    def score(key, column, order):
        ...
        with col2:
            ...
            select_order = st.selectbox(
                "Order", ["Ascending", "Descending"], index=idx, key=key
            )
